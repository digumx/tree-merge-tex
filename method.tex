...
\dmcmt{Alternate section. Following review 1, I think it may be better to say
    upfront over here that we can do this in various ways, and describe these a
bit.}
...
\dmcmt{Should we talk about pgd-based things at all? It may not add much to the
story, and the experiments using these I feel don't provide much insight beyond
saying that pgd does not work. Ofc we may explain why that is the case, and try
to understand that, but does that add much to the story?}
...
%\dmcmt{Also in general, should we be talking about these different methods at
%    all? We are not able to demonstrate much difference about this in our
%experiments. In fact, the randomized baseline seems to perform quite well}
