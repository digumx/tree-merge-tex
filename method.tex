...
\footnote{ Generally, inputs to \dnn
    are bounded, and therefore so is $P$, ensuring that the $L_2$ norm is well
defined.}
...
we use a uniform sampling of the input region where $P$ holds. Since generally
the input region is, in our case, interval, such a sampling is straightforward,
however our method generalizes to other input regions via other sampling methods
as well.
