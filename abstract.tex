...
applications tend to rely on very large DNNs to
achieve the desired accuracy, making it a challenge for them to be executed on
resource-constrained and real-time settings.
The size of these networks is also a bottleneck in proving their trustworthiness
through formal verification or explanation, limiting the deploy-ability of these
networks in safety-critical domains. Therefore, it is imperative to be able to
compress these networks while maintaining a strong formal connection and
preserving desirable safety properties.
Several \emph{syntactic} abstraction techniques have been proposed that produce
an abstract network with a formal guarantee that safety properties will be
preserved.
These, however, do not take the \emph{semantic} behavior of the network and thus
produce sub-optimally large networks. \dmcmt{This is instead of saying that they
don't guarantee \abs smaller than \cnc. I am not saying that since we don't
guarantee / demonstrate that either. Is that okay?} On the other hand,
compression and \emph{semantic} abstraction techniques have been proposed which
achieve significant reduction in network size but only weakly preserve a limited
set of safety properties. 
In this paper, we propose to combine the semantic and
...
Our experiments on standard neural network benchmarks show that this can produce
smaller abstract networks than existing methods while preserving safety
properties.
