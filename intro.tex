...
\cite{backdoor-poisoning} etc. \todo{cite} and, being generally un-interpretable, can often produce 
unexpected behaviours. Therefore, to build trust on safety critical systems 
that utilize \dnn, it is \dmcmt{has become?} critical to understand, interpret 
and validate the possible behaviors of these \dnn via formal analysis.
\dmcmt{ Cite some dnn verif case studies, like nn-lander-verif,
Prof Deepak's work, etc? Mention verification / interpretability here?}
\sncmt{Verifying generalization in 
deep neural networks- Aurora benchmark for congestion control. Lunar lander
verifying generalizations in neural networks}
...
minimal-image-fxai} and others \todo{cite}. While verification of neural networks 
...
thus producing potentially sub-optimal abstractions. \dmcmt{This last bit is not necessarily exposed by our
experiments..}
...
is not strong \dmcmt{wording okay?} enough to be effective \dmcmt{Avoiding language like
...
\dmcmt{The next 3 paras, esp the next para, are too verbose. Shorten}
...
\dmcmt{Mention restriction to fully connected feed forward relu dnn somewhere?}

\dmcmt{We need a word for the generalized notion of over-approximate-ness of our
   abstraction, I've used 'weakness' and 'quality' here, what should we have as
a word in general?}\sncmt{Use 'quality'}
