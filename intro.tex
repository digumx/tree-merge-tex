...
neural network verification techniques, they are not guaranteed to produce \abs
that is smaller in size than \cnc \cite{cegar-nn}. 
\dmcmt{We also don't guarantee \abs smaller
in size than \cnc. Given this, is it okay to say this? Maybe we can claim that
they are relatively bigger than the compression methods, which is in general
true?}
This prevents these techniques from being useful for compressing \dnn for
deployment in safety-critical resource-constrained environments.
...
property. Secondly, we use it \dmcmt{Maybe remove}
