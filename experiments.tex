...
adversarial robustness properties and \acasxu with both original properties from
\cite{reluplex} and adversarial properties discussed in \cite{cegar-nn}.
Figure \ref{f:ncex} shows some typical plots
for these experiments. Here the x-axis measures the reduction rate with respect
to the \inc-\dec split networks and the y-axis is one minus the rate of true
counterexamples within a random sample - thus a high value indicates a good
abstraction.
...
networks and properties and attempt to verify them using a \cegar approach. We use our
...
Additionally, we observe that our approach makes very few number of 
solver calls (Table \ref{t:acas-verif}). This is unlike previous methods that
relied on heuristics to guide refinement order and arbitrarily choose the 
number of refinement steps \cite{cegar-nn} to ensure not too many solver calls 
take place. On the other hand we precisely take the number of refine steps 
which are necessary between each solver call that are required to 
eliminate one particular counterexample. This optimality comes from 
our framework's utilization of semantic closeness factor ($\cls$) to 
unambiguously order refinement operations. 
