...
\dmcmt{The motivation behind defining critical classes was not clear to one
    reviewer. Therefore, I have changed this section to start with motivating
compression, then talk about critical classes and formal guarantees}
...
class `0' as the critical class. Then, starting with the fully merged network, we
...
\ref{s:finding-gamma}. We use the \abcrown implementation for \pgd.
...
respectively. In all graphs the Y-axis shows true/false negative/positive rates
(see legend) and the X-axis shows reduction rates. 
\todo{Add labels on graphs here}}
    \label{f:mnist-class}
\end{figure}
...
$2\times256$ & Uniform-Score      & 45.7 \%   & 0.3  \%         &  326    \\ 
$4\times256$ & Uniform-Score      & 38.7 \%   & 2.2  \%         & 1037    \\ 
$6\times256$ & Uniform-Score      & 45.5 \%   & 0.2  \%         & 2425    \\ 
$2\times256$ & PGD-Score          & 53.4 \%   & 95.6 \%         &   85    \\ 
$4\times256$ & PGD-Score          & 96.9 \%   & 100  \%         &    0    \\ 
$6\times256$ & PGD-Score          & 30.6 \%   & 94.1 \%         & 1482    \\ 
$2\times256$ & Samples-PGD-Score  & 46.0 \%   & 0.5  \%         &  316    \\ 
$4\times256$ & Samples-PGD-Score  & 39.2 \%   & 2.1  \%         & 1027    \\ 
$6\times256$ & Samples-PGD-Score  & 45.5 \%   & 0.2  \%         & 2425    \\ 
\hline
\end{tabular}
...
but may continue to refine beyond this point. \dmcmt{Put this last sentence as a
footnote?} 

\dmcmt{Intuitively, we can claim that we are seeing two things here: one is
    comparing $\gamma$ selection strategies, and another is seeing if doing the
    tree based refinement helps at all. Both the gamma selection and the tree
    makes use of various kinds of sampling, so both could be capturing some
    global semantic info. The former we compare in this table, and we find that
    mostly, $\gamma$ selection does not matter. For the latter question, we
    would need some new experiments comparing with other methods of compression
(maybe some implementation of guy katz?) }

Overall, we find that (with the exception of PGD-Score) the quality of the
networks are similar across the various strategies, while the number of steps
taken to arrive at the network is lower for each strategy compared to the Random
...
We find that \textit{pgd} performs relatively poorly, with fewer
...
implementation of \pgd fails to find \gencex early on in the
refinement process for these instances, leading to early termination. 

\dmcmt{In line with following para, and discussion in sec 3.3.1, should we even
look at pgd?}
