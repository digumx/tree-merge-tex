...
\kmcmt{Not sure what you mean? Do you mean that we should calculate the refinement time separately? I am not sure if that is required. I mean, it is good to not take too much time to refine. But then the number of refinement steps and the quality of refinement must also be considered along with the time. For example, one can refine very quickly by simply falling back to the original network immediately. But that is no good. So, if the time is less, and also the number of iterations and the size of the final refined network is less then we can report, otherwise we need not.}
...
\kmcmt{Let us change the order here. Let us keep the strongest subsection first. And let us move the ACAS Xu verification subsection to the end.}
...
be a lot lower.} 
\kmcmt{This is fine. Let us just report whatever we get.}
...
\dmcmt{But what about the safe cases, what do we say?}. \kmcmt{We need not talk about safe cases if we do not have anything to say. That's fine.}  In fact, for most
...
in the appendix?} \kmcmt{Yes, we should have the full table in the appendix and a link to that here.} Note that since we use only two classes in our
...
split is not clear from the table}. \kmcmt{We can claim this without recreating anything. We can simply remark that this was also observed there, and we believe that it for the same reason.. but let's discuss what we want to convey from this para and if we can state that in an easier way.} For other instances, the timeout happened
...
something negative about our abstraction?} \kmcmt{Yes, it does. Let us first decide what is the point that we wish to make. And then we will see how to make that point.}
...
and report this precisely?} \kmcmt{I think it would be good to do that. It will help the
point that we are making. What do you think?} We see that the time taken by the solver call on
...
\kmcmt{No. I think the abstraction can still be argued as useful, unless we start seeing several instances where the solver time consistently increased after the abstraction.}

\subsection{Robustness on \mnist}
\dmcmt{What about doing these experiments on ACAS?}
\dmcmt{Do pgd then samples}
\kmcmt{We can do this in the end if time permits.}
...
in terms or spurious counterexample rate instead?} \kmcmt{We can do that. Essentially, whichever way makes the plots look better. But they are the same thing. So, let us not worry too much about this right now.} We see that as we refine the
...
\dmcmt{Is this expl okay?} \kmcmt{Yes, this is fine.}
...
these?} \kmcmt{If space permits; looks unlikely as of now.}
...
what Ive written vs global view angle.} \kmcmt{These two paragraphs look okay to me. Let us discuss
the specific concerns when we meet.}
...
} \kmcmt{No, not at this point. We should have talked about this earlier itself so that it is very clear at this point.} connecting the behavior
...
the same} \kmcmt{That is okay; a likely explanation is fine.} 
