...
%\subsection{Preserving Safety Properties on Neural Network Abstractions}
%\label{s:conc-abs}
%
%The notion of preserving safety properties when going from \cnc to \abs
%has been formalized in \cite{cegar-nn} as follows:
%Any general neural network safety property $(P, \mcnc, Q)$ where $Q$ is a
%conjunction of linear constraints (a situation that occurs commonly in practice)
%can be converted to an equivalent property of the form $(P, \mcnc, y < c)$ for
%some $c$ by appending extra layers to \cnc encoding the postcondition $Q$. Then,
%one can ensure that 
%
%In particular, guarantees of the
...
%\kmcmt{Let us use these references for the 2-class
%classification:~\cite{chauhan2022efficiently,liu2022abstraction,10.1145/3644387}.
%Also, I think it might be better to state the 2-class directly as a
%modification. Something like, ``Instead of the four-way split, we consider only
%a two-way split of neurons as \emph{inc} and \emph{dec}.'' And then let us
%preserve the term ``re-merging'' for merging an increment and its corresponding
%decrement neuron back into one. I think the following para, and possibly
%relevant parts of the following sections, will have to be changed accordingly.}
